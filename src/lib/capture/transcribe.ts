...
import { buildTranscribeUrl, optionsFromEnv } from "./transcribe-url";
 ...
 * Quality knobs and their rationale live in transcribe-url.ts.
 */
export async function transcribeWav(wav: Uint8Array<ArrayBuffer>): Promise<DeepgramUtterance[]> {
  ...
  const res = await fetch(buildTranscribeUrl(optionsFromEnv(process.env)), {
    method: "POST",
