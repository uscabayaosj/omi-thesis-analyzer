 ...
 *  - diarize + utterances: speaker-attributed turns with tight timestamps,
 *    which transcribe-map.ts maps back to wall-clock time.
 ...
 *  - mip_opt_out: recordings of research participants are not shared with
 *    Deepgram's model-improvement program (small per-minute surcharge).
...
export function buildTranscribeUrl(opts: TranscribeUrlOptions = {}): string {
  ...
  p.set("mip_opt_out", String(opts.mipOptOut ?? true));
