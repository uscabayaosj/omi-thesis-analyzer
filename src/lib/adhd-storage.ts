...
// ── low-level ──
// Generic map persistence lives in map-storage.ts; the wrapper below exists
// only to keep the PWA icon badge in step with unacknowledged commitments the
// moment they change (including after a quota-pruned retry), not just on next
// app open.
