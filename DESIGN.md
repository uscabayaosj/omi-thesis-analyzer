...
- **Parity Action:** for two or more independent, co-equal actions sharing one control (e.g. the selection toolbar's Group Thesis / Run ADHD — different actions, not a preference order), no button claims solid-fill indigo, since only one action per screen is allowed that mark under the One Ink Rule. All of them share one class instead: flat slate when disabled, and an indigo-tinted **ready** state (`border-indigo-500/50 bg-indigo-950/40 text-indigo-200`, `hover:` one step up) when enabled — the same wash `ConversationRow` uses for a selected item, not a new pattern. Indigo still means "ready to act," it just isn't spent on declaring one of several parallel choices more important than the others. Disabled state is `bg-slate-700` here, not `bg-slate-800` — this toolbar is itself a `.card` (Ink Panel), so the Secondary Button "shift one step inside a card" rule applies just as much to this variant.
